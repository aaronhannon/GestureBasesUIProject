...
public class GestureEventArgs : EventArgs
{
    ...
    public GestureEventArgs(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, string gestureID)
    {
        ...
        Debug.Log("Confidence: " + detectionConfidence);
...
public class GestureDetector : IDisposable
{
    ...
    private readonly string jumpDB = Path.Combine(Application.streamingAssetsPath, "GestureDB/JumpDB.gbd");
    ...
    private VisualGestureBuilderDatabase _dbGestures;
    ...
    public GestureDetector(KinectSensor kinectSensor)
    {
        ...
        _dbGestures= VisualGestureBuilderDatabase.Create(this.jumpDB);
        
            // we could load all available gestures in the database with a call to vgbFrameSource.AddGestures(database.AvailableGestures), 
            // but for this program, we only want to track one discrete gesture from the database, so we'll load it by name
        foreach (Gesture gesture in _dbGestures.AvailableGestures)
        {
        ...
        Debug.Log("GestureDetectorCreated");
    }
